...
3. Process the posts based on the rules, and create/update the RSS file accordingly.
    - Add new posts based on the rules below.
    - Update existing posts when fetched post content differs.
    - Skip new posts based on the rules below.
    - Remove posts from the existing file based on the rules below.
    - Prune the RSS file based on the maximum retention age.
...
2. Existing post(s) were updated.
3. Existing post(s) were removed.
4. The RSS file was pruned based on maximum retention age.

OTHERWISE revert/undo the changes to the RSS file so the current git repo state is unaffected.

## VALID POST RULES

- Valid new posts are not in the existing RSS file, and have dates at or newer than the maximum post age.
- Valid posts are not subscriber-only posts.
- Valid posts are not paywalled posts.
...
- DO NOT prefer existing RSS feeds when present.
...
- Always include ALL post content; DO NOT create shorter excerpts or otherwise abridge the post content.
    - If existing posts are using shorter content excerpts, update them to use tghe full content.
